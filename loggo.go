...
func initLog(debugProcess,infoProcess,warningProcess,errorProcess  io.Writer) {
	...
	Warn=log.New(warningProcess, "WARNING: ",log.Ldate|log.Ltime|log.Lshortfile)

	Err=log.New(errorProcess, "ERROR: ", log.Ldate|log.Ltime|log.Lshortfile)
}

func logConfig(configfile string)  Log_variable{
...
func RunLog(configFile string) {

	file_name:=logConfig(configFile).FILE_NAME
	dir_name:=logConfig(configFile).DIR_NAME
	dir_name_info:=logConfig(configFile).DIR_NAME_INFO
	dir_name_error:=logConfig(configFile).DIR_NAME_ERROR
	debugMode:=logConfig(configFile).DEBUGMODE
